...
def ngs_mutect(inputs, targets_data_processed):
	...
	for s in targets_data_processed:
		...
		if(os.path.exists(tumorFile) and os.path.exists(normalFile)):
			...
			if inputs.verbose:
				print "[INFO:COMMAND] java -jar %s AddOrReplaceReadGroups I=%s O=%s RGID=%s RGLB=%s RGPL=%s RGPU=%s RGSM=%s" % (inputs.picard,s._bamCalibTumor,tumorFile,s._id + '_Tumor_Lib1','Lib1','ILLUMINA',s._id + '_Tumor_Lib1',s._id + '_Tumor')
			if not inputs.dry:
				ret_val = os.system("java -jar %s AddOrReplaceReadGroups I=%s O=%s RGID=%s RGLB=%s RGPL=%s RGPU=%s RGSM=%s" % (inputs.picard,s._bamCalibTumor,tumorFile,s._id + '_Tumor_Lib1','Lib1','ILLUMINA',s._id + '_Tumor_Lib1',s._id + '_Tumor'))
			if inputs.verbose:
				print "[INFO:COMMAND] java -jar %s AddOrReplaceReadGroups I=%s O=%s RGID=%s RGLB=%s RGPL=%s RGPU=%s RGSM=%s" % (inputs.picard,s._bamCalibNormal,normalFile,s._id + '_Normal_Lib2','Lib2','ILLUMINA',s._id + '_Normal_Lib2',s._id + '_Normal')
			if not inputs.dry:
				ret_val = os.system("java -jar %s AddOrReplaceReadGroups I=%s O=%s RGID=%s RGLB=%s RGPL=%s RGPU=%s RGSM=%s" % (inputs.picard,s._bamCalibNormal,normalFile,s._id + '_Normal_Lib2','Lib2','ILLUMINA',s._id + '_Normal_Lib2',s._id + '_Normal'))
